// encryption/server/server_tcp_enc.c
...
// For files ≤ 100MB use 1KB chunks; otherwise, use 1MB chunks.
#define CHUNK_SIZE_SMALL 1024
#define CHUNK_SIZE_LARGE (1024*1024)

double DROP_PROB = 0.2;     // 20% chance to drop a chunk (simulation)
double CORRUPT_PROB = 0.1;  // 10% chance to corrupt a chunk

// --- Helper: send_all and recv_all ---
...
// --- Basic message framing with 4-byte length prefix ---
...
// --- Secure send and receive wrappers ---
...
int main(void) {
    ...
    printf("TCP Server (encrypted) listening on port %d...\n", TCP_PORT);
    ...
    if(!rsa_priv) { fprintf(stderr, "Error loading RSA private key\n"); exit(1); }

    // Read server's public key from file.
    FILE *pub_fp = fopen("server_public.pem", "r");
    if(!pub_fp) { perror("fopen public key"); exit(1); }
    ...
    // Receive the AES key encrypted with RSA from the client.
    ...
    unsigned char aes_key[32];  // expecting a 256-bit AES key
    ...
    uint32_t file_len;
    if(secure_recv_msg(conn, &file_data, &file_len, aes_key, iv) < 0) {
    ...
    // --- Securely send header ---
    uint32_t header_len = 4 + 64; // 4 bytes for total_chunks, 64 for overall checksum hex
    unsigned char *header = malloc(header_len);
    if(!header) { perror("malloc header"); exit(1); }
    ...
    if(!indices) { perror("malloc indices"); exit(1); }
    for(uint32_t i = 0; i < total_chunks; i++)
        indices[i] = i;
    for(uint32_t i = 0; i < total_chunks; i++) {
    ...
    // Cleanup: free allocated chunk data.
    for(uint32_t i = 0; i < total_chunks; i++) {
        free(chunks[i].data);
    }
    free(chunks);
